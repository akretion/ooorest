  ...
  class AbstractModel
    ...
    class << self
      ...
      def new(m)
        ...
        puts "[Ooorest] Could not load model #{m}, assuming model is non existing. (#{$!})" unless Rails.env.test?
        nil
      end
    ...
    def initialize(model_or_model_name)
      ...
      if model.ancestors.map(&:to_s).include?('ActiveRecord::Base') && !model.abstract_class?
        ...
        require 'ooorest/adapters/active_record'
        extend Adapters::ActiveRecord
      elsif model.ancestors.map(&:to_s).include?('Mongoid::Document')
        # Mongoid
        @adapter = :mongoid
        require 'ooorest/adapters/mongoid'
