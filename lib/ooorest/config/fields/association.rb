...
module Ooorest
  module Config
    module Fields
      class Association < Ooorest::Config::Fields::Base
        ...
        register_instance_option :pretty_value do
          ...
          [value].flatten.select(&:present?).map do |associated|
            amc = polymorphic? ? Ooorest.config(associated) : associated_model_config # perf optimization for non-polymorphic associations
        ...
        def associated_model_config
          @associated_model_config ||= Ooorest.config(association[:model_proc].call)
        end
