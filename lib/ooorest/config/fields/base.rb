...
require 'ooorest/config/groupable'
  ...
  module Config
    module Fields
      class Base
        ...
        include Ooorest::Config::Groupable
        ...
        register_instance_option :search_operator do
          @search_operator ||= Ooorest::Config.default_search_operator
        end
        ...
        #
        # @see Ooorest::AbstractModel.properties
        ...
        register_instance_option :visible? do
          returned = true
          (Ooorest.config.default_hidden_fields || {}).each do |section, fields|
            if self.section.is_a?("Ooorest::Config::Sections::#{section.to_s.camelize}".constantize)
        ...
        register_instance_option :render do
          bindings[:view].render :partial => "ooorest/main/#{partial}", :locals => {:field => self, :form => bindings[:form] }
        end
        ...
        def editable?
          ...
          if Ooorest::Config.yell_for_non_accessible_fields
            accessible = "attr_accessible :#{self.method_name}#{role == :default ? '' : ", :as => :#{role}"}"

            Rails.logger.debug <<-MESSAGE.strip_heredoc


            [Ooorest] Please add '#{accessible}' in your '#{bindings[:object].class}' model definition if you want to make it editable.
            ...
            Add 'config.yell_for_non_accessible_fields = false' in your 'ooorest.rb' initializer if you do not want to see these warnings
        ...
        def association?
          kind_of?(Ooorest::Config::Fields::Association)
        end
        ...
        #
        # @see Ooorest::Config::Fields::Base.register_instance_option :required?
        def optional?
          not required?
        end
        ...
        #
        # @see Ooorest::Config::Fields::Base.optional
        def optional=(state)
          optional(state)
        end
