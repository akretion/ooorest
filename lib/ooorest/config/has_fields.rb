    ...
    module HasFields
      ...
      def field(name, type = nil, add_to_section = true, &block)
        ...
        if field.nil? && type.nil?
          field = (_fields << Ooorest::Config::Fields::Types.load(:string).new(self, name, {})).last
        ...
        # one found in default stack
          ...
          field = (_fields <<  Ooorest::Config::Fields::Types.load(type).new(self, name, properties)).last
        end
      ...
      def _fields(readonly = false)
        ...
        unless self.class == Ooorest::Config::Sections::Base
          # parent is Ooorest::Config::Model, recursion is on Section's classes
          @_ro_fields ||= parent.send(self.class.superclass.to_s.underscore.split('/').last)._fields(true).freeze
        else # recursion tail
          @_ro_fields = @_fields = Ooorest::Config::Fields.factory(self)
        end
