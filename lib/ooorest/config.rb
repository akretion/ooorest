require 'ooorest/config/lazy_model'
require 'ooorest/config/sections/list'
require 'active_support/core_ext/class/attribute_accessors'

module Ooorest
  module Config
    # Ooorest is setup to try and authenticate with warden
    # If warden is found, then it will try to authenticate
    #
    # This is valid for custom warden setups, and also devise
    # If you're using the admin setup for devise, you should set Ooorest to use the admin
    #
    # @see Ooorest::Config.authenticate_with
    # @see Ooorest::Config.authorize_with
    ...
    class << self
      ...
      # Configuration option to specify a whitelist of models you want to Ooorest to work with.
      # The excluded_models list applies against the whitelist as well and further reduces the models
      # Ooorest will use.
      # If included_models is left empty ([]), then Ooorest will automatically use all the models
      ...
      #   end
      #
      # @see Ooorest::Config::DEFAULT_AUTHENTICATION
      ...
      def audit_with(*args, &block)
        ...
        if(extension)
          @audit = Proc.new {
            @auditing_adapter = Ooorest::AUDITING_ADAPTERS[extension].new(*([self] + args).compact)
          }
        else
      ...
      #   end
      #
      # See the wiki[https://github.com/sferik/ooorest/wiki] for more on authorization.
      #
      # @see Ooorest::Config::DEFAULT_AUTHORIZE
      def authorize_with(*args, &block)
        extension = args.shift
        if(extension)
          @authorize = Proc.new {
            @authorization_adapter = Ooorest::AUTHORIZATION_ADAPTERS[extension].new(*([self] + args).compact)
          }
        else
      ...
      def configure_with(extension)
        configuration = Ooorest::CONFIGURATION_ADAPTERS[extension].new
      ...
      #   end
      #
      # @see Ooorest::Config::DEFAULT_CURRENT_USER
      ...
      def models_pool
        ...
        excluded = (excluded_models.map(&:to_s) + ['Ooorest::History'])
      ...
      # its class name as a string or symbol or a Ooorest::AbstractModel
      # instance.
      #
      ...
      def model(entity, &block)
        ...
        if block
          @registry[key] = Ooorest::Config::LazyModel.new(entity, &block)
        else
          @registry[key] ||= Ooorest::Config::LazyModel.new(entity)
        end
      end
      ...
      def actions(&block)
        Ooorest::Config::Actions.instance_eval(&block) if block
      end
      ...
      # @see Ooorest::Config.registry
      def models
        Ooorest::AbstractModel.all.map{|m| model(m)}
      end
